        ...
        <div class="row q-mb-md bg-grey-2 items-center q-pl-xs">
          ...
          <div class="col-3 text-grey">
            <q-input label="상세주소" filled dense v-model="address2"
                     color="blue-grey-9 q-ma-xs" ref="phone">
...
export default {
  ...
  methods: {
    onClickCreate () {
      this.msg = '생성하시겠습니까?';
      this.confirmMethod = this.createUser;
      this.isConfirm = true;
    },
      ...
      if(!this.email) {
        ...
        const body = {
          user: {
            ...
            avatar: '',
            agreeMarketingPhone: true,
            agreeMarketingSMS: true,
            agreeMarketingEmail: true,
            agreeMarketingPost: true,
          },
          pets: [],
        };

        for(let i = 0; i < this.petsNum; i++) {
          const pet = this.petsRow[i];
          const petData = {
            type: pet.petsType,
            gender: pet.petsGender,
            castration: pet.neutralization,
            breed: '',
            breedKR: '',
            breedEN: '',
            birth: new Date(`${pet.birthYear}-${pet.birthMonth}`),
          };
          body.pets.push(petData);
        }

        const apiResult = await API.addUser(body);
